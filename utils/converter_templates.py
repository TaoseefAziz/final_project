...
def replace_column(df: pd.DataFrame, column: str,
    ...
    """
    Replace column with one or more columns, each with a value extracted via regex from the values of the
    original column
    ...
    :param new_cols_and_funcs: keyword arguments, one for each new column. For each column, the column name
        will be the argument's name, and the values will be extracted from the original column using the regex
        specified by the argument's value.
